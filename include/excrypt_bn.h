...
// excrypt_bn_mod.cpp
void ExCryptBnQwNeModMul(const uint64_t* input_A, const uint64_t* input_B, uint64_t* output_C,
                         uint64_t inverse, const uint64_t* modulus, uint32_t modulus_size);
...
// BOOL ExCryptBnQwBeSigCreate(EXCRYPT_SIG* sig, const uint8_t* hash, const uint8_t* salt, const
// EXCRYPT_RSA* privkey);
BOOL ExCryptBnQwBeSigVerify(EXCRYPT_SIG* sig, const uint8_t* hash, const uint8_t* salt,
                            const EXCRYPT_RSA* pubkey);
int32_t ExCryptBnQwBeSigDifference(EXCRYPT_SIG* sig, const uint8_t* hash, const uint8_t* salt,
                                   const EXCRYPT_RSA* pubkey);
...
// excrypt_bn_rsa.cpp
// TODO: investigate PrivExp/'D' constant, XeCrypt struct doesn't provide it, but pretty much all
// RSA codebases need it We only have one for 1024-bit numbers atm, should be some way to use ModInv
// to calculate it though...
...
void ExCryptBnDwLePkcs1Format(const uint8_t* hash, uint32_t format, uint8_t* output_sig,
                              uint32_t output_sig_size);
BOOL ExCryptBnDwLePkcs1Verify(const uint8_t* hash, const uint8_t* input_sig,
                              uint32_t input_sig_size);

// BOOL ExCryptBnQwNeRsaKeyGen(uint32_t num_bits, uint32_t public_exponent, EXCRYPT_RSA* pub_key,
// EXCRYPT_RSA* priv_key);
