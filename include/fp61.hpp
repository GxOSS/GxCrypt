...
FP61_FORCE_INLINE uint64_t Emulate64x64to128(uint64_t& r_hi, const uint64_t x, const uint64_t y) {
    ...
    */
    ...
    const uint64_t middle = p10 + static_cast<uint32_t>(p00 >> 32) + static_cast<uint32_t>(p01);
    ...
    */
    ...
    // 64-bit product + two 32-bit values
    r_hi = p11 + static_cast<uint32_t>(middle >> 32) + static_cast<uint32_t>(p01 >> 32);
...
#define CAT_MUL128(r_hi, r_lo, x, y) r_lo = _umul128(x, y, &(r_hi));
...
#define CAT_MUL128(r_hi, r_lo, x, y) r_lo = Emulate64x64to128(r_hi, x, y);
