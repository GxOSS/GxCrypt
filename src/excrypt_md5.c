...
void md5_process_block(EXCRYPT_MD5_STATE* state) {
    ...
    for (i = 0, j = 0; i < 16; ++i, j += 4)
        m[i] = (state->buffer[j]) + (state->buffer[j + 1] << 8) + (state->buffer[j + 2] << 16) +
               (state->buffer[j + 3] << 24);
...
void ExCryptMd5(const uint8_t* input1, uint32_t input1_size, const uint8_t* input2,
                uint32_t input2_size, const uint8_t* input3, uint32_t input3_size, uint8_t* output,
...
void ExCryptHmacMd5(const uint8_t* key, uint32_t key_size, const uint8_t* input1,
...
uint8_t ExCryptHmacMd5Verify(const uint8_t* key, uint32_t key_size, const uint8_t* input1,
                             uint32_t input1_size, const uint8_t* input2, uint32_t input2_size,
                             const uint8_t* input3, uint32_t input3_size,
                             const uint8_t* compare_buf, uint32_t compare_buf_size) {
    uint8_t output[0x14];
    ExCryptHmacMd5(key, key_size, input1, input1_size, input2, input2_size, input3, input3_size,
                   output, 0x14);
