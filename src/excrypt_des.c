...
void ExCryptDesEcb(const EXCRYPT_DES_STATE* state, const uint8_t* input, uint8_t* output,
                   uint8_t encrypt) {
...
void ExCryptDesCbc(const EXCRYPT_DES_STATE* state, const uint8_t* input, uint32_t input_size,
...
void ExCryptDes3Ecb(const EXCRYPT_DES3_STATE* state, const uint8_t* input, uint8_t* output,
                    uint8_t encrypt) {
    if (encrypt) {
...
void ExCryptDes3Cbc(const EXCRYPT_DES3_STATE* state, const uint8_t* input, uint32_t input_size,
                    uint8_t* output, uint8_t* feed, uint8_t encrypt) {
    uint64_t last_block = *(uint64_t*) feed;
    for (uint32_t i = 0; i < input_size / 8; i++) {
        if (encrypt) {
