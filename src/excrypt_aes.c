...
void rijndaelEncrypt_AESNI(const uint32_t* rk, int nrounds, const uint8_t* plaintext,
                           uint8_t* ciphertext) {
...
void rijndaelDecrypt_AESNI(const uint32_t* rk, int nrounds, const uint8_t* ciphertext,
                           uint8_t* plaintext) {
...
void ExCryptAesKey(EXCRYPT_AES_STATE* state, const uint8_t* key) {
    if (aesni_supported || aesni_get_supported()) {
        ...
        // For some implementation reasons, decryption key schedule is NOT the encryption key
...
void ExCryptAesEcb(const EXCRYPT_AES_STATE* state, const uint8_t* input, uint8_t* output,
                   uint8_t encrypt) {
    if (encrypt) {
...
void rijndaelCbcEncrypt(const uint32_t* rk, const uint8_t* input, uint32_t input_size,
                        uint8_t* output, uint8_t* feed) {
    uint8_t* iv = feed;
...
void rijndaelCbcDecrypt(const uint32_t* rk, const uint8_t* input, uint32_t input_size,
                        uint8_t* output, uint8_t* feed) {
...
void ExCryptAesCbc(const EXCRYPT_AES_STATE* state, const uint8_t* input, uint32_t input_size,
                   uint8_t* output, uint8_t* feed, uint8_t encrypt) {
    if (encrypt) {
...
uint32_t* aesschedule_dectable(EXCRYPT_AES_SCHEDULE* state) {
    return (uint32_t*) &state
        ->keytab[state->num_rounds + 1]; // dec table starts at last entry of enc table
}

void ExCryptAesCreateKeySchedule(const uint8_t* key, uint32_t key_size,
                                 EXCRYPT_AES_SCHEDULE* state) {
...
void ExCryptAesCbcEncrypt(EXCRYPT_AES_SCHEDULE* state, const uint8_t* input, uint32_t input_size,
                          uint8_t* output, uint8_t* feed) {
    uint8_t* iv = feed;
    for (uint32_t i = 0; i < input_size; i += AES_BLOCKLEN) {
        xorWithIv(input, output, iv);
...
void ExCryptAesCbcDecrypt(EXCRYPT_AES_SCHEDULE* state, const uint8_t* input, uint32_t input_size,
                          uint8_t* output, uint8_t* feed) {
    uint8_t current_input[AES_BLOCKLEN];
    uint8_t iv[AES_BLOCKLEN];
    memcpy(iv, feed, AES_BLOCKLEN);
