...
uint8_t kRoamableObfuscationKey_Retail[0x10] = {0xE1, 0xBC, 0x15, 0x9C, 0x73, 0xB1, 0xEA, 0xE9,
                                                0xAB, 0x31, 0x70, 0xF3, 0xAD, 0x47, 0xEB, 0xF3};
uint8_t kRoamableObfuscationKey_Devkit[0x10] = {0xDA, 0xB6, 0x9A, 0xD9, 0x8E, 0x28, 0x76, 0x4F,
...
void ExKeysKeyVaultSetup() {
    // ROAMABLE_OBFUSCATION_KEY doesn't seem to be stored in the keyvault, is generated at
    // runtime/stored in HV?
...
BOOL ExKeysIsKeySupported(uint32_t key_idx) {
    return (ExKeysKeyVaultLoaded() && kExKeyProperties.count(key_idx) > 0) ||
           ExImportedKeys.count(key_idx) > 0;
}
...
BOOL ExKeysQwNeRsaPrvCrypt(uint32_t key_idx, const uint64_t* input, uint64_t* output) {
    if (key_idx != XEKEY_CONSOLE_PRIVATE_KEY && key_idx != XEKEY_XEIKA_PRIVATE_KEY &&
        key_idx != XEKEY_CARDEA_PRIVATE_KEY)
        return false;

    // Xeika key is larger than the others, and likely needs a different D/PrivExp value for it (see
    // kStaticPrivateExponent1024), so disallow it for now
...
// Signs the given hash with the loaded keyvaults private-key, and writes out console cert +
// signature to output_cert_sig
...
// Verifies that the given hash was signed by the given console certificate, and that the console
// certificate was signed by the master key.
BOOL ExKeysConsoleSignatureVerification(const uint8_t* hash, uint8_t* input_signature,
    ...
    if (master_key_size == 0x110 && _byteswap_ulong(*(uint32_t*) master_key) == 0x20) {
        ...
        if (ExKeysPkcs1Verify(cert_checksum, input_signature + 0xA8, (EXCRYPT_RSA*) master_key)) {
            // The certificate doesn't have the public key size - the real function does this stuff,
            // too.
            console_public_key.rsa.num_digits = _byteswap_ulong(0x10);
            console_public_key.rsa.pub_exponent = *(uint32_t*) (input_signature + 0x24);
            memcpy(console_public_key.modulus, input_signature + 0x28,
                   sizeof(console_public_key.modulus));
...
uint32_t ExKeysHmacShaUsingKey(const uint8_t* obscured_key, const uint8_t* input1,
                               uint32_t input1_size, const uint8_t* input2, uint32_t input2_size,
                               const uint8_t* input3, uint32_t input3_size, uint8_t* output,
    ...
    ExCryptHmacSha(key, 0x10, input1, input1_size, input2, input2_size, input3, input3_size, output,
                   output_size);

    return 0; // TODO: X_STATUS_SUCCESS
}

uint32_t ExKeysHmacSha(uint32_t key_idx, const uint8_t* input1, uint32_t input1_size,
                       const uint8_t* input2, uint32_t input2_size, const uint8_t* input3,
                       uint32_t input3_size, uint8_t* output, uint32_t output_size) {
    ...
    ExCryptHmacSha(key, size, input1, input1_size, input2, input2_size, input3, input3_size, output,
                   output_size);

    return 0; // TODO: X_STATUS_SUCCESS
}

uint32_t ExKeysObfuscate(BOOL roaming, const uint8_t* input, uint32_t input_size, uint8_t* output,
                         uint32_t* output_size) {
    ...
    *output_size = input_size + 0x18;
    ...
    uint32_t key_idx = roaming ? uint32_t(XEKEY_ROAMABLE_OBFUSCATION_KEY)
                               : uint32_t(XEKEY_CONSOLE_OBFUSCATION_KEY);

    auto result = ExKeysHmacSha(key_idx, output + 0x10, *output_size - 0x10, nullptr, 0, nullptr, 0,
                                output, 0x10);
    if (result < 0)
        return result;
...
BOOL ExKeysUnObfuscate(BOOL roaming, const uint8_t* input, uint32_t input_size, uint8_t* output,
                       uint32_t* output_size) {
    if (input_size < 0x18)
        return false;
