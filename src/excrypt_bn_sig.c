...
// BnQwBeSig seems to be a custom XeCrypt signature format, so we have to implement code for it
// ourselves :( AFAIK console-signing (eg. thru XeKeysConsolePrivateKeySign) use PKCS1 instead, so
// hopefully we can use some existing codebase for those.
